    ...
    <div className={`${className} flex flex-row gap-2`}>
      <DefaultTooltip text={t("general.goHome")}>
        <button type="button" onClick={() => navigateTo("/")}>
          <HomeIcon className="my-auto w-5 h-5 text-gray-400 dark:text-gray-400" />
        </button>
      </DefaultTooltip>
      {levels.map((l) => (
        <div key={l} className="flex flex-row gap-2">
          <ChevronRightIcon className="my-auto w-5 h-5 text-gray-600 dark:text-gray-500" />
          <p className="my-auto text-sm leading-5 font-normal text-gray-500 dark:text-gray-400">
            {l}
          </p>
        </div>
      ))}
