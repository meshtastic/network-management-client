...
pub fn load_graph(packets: Vec<NeighborInfo>, loc_hashmap: HashMap<u32, MeshNode>) -> Graph {
    ...
    for packet in packets {
        ...
        for neighbor in packet.neighbors {
            ...
            let node_data = loc_hashmap.get_mut(&packet.id).unwrap();
            let neighbor_data = loc_hashmap.get_mut(&neighbor.id).unwrap();
            let node_position = node_data.data.position.unwrap();
            let neighbor_position = neighbor_data.data.position.unwrap();
...
mod tests {
    ...
    fn test_init_graph() {
        ...
        let loc_hashmap: HashMap<u32, MeshNode> = HashMap::new();
